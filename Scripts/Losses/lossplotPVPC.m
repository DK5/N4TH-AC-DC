...
h = figure();surf(X,Y,data.lossPVPC','FaceColor','interp','FaceLighting','gouraud');
title (data.runtitle, 'Interpreter', 'none','Fontsize',14); 
axis ([min(data.iAC) max(data.iAC) min(data.frequency) max(data.frequency) 1.1*min(min(data.lossPVPC,[],1)) 1.1*max(max(data.lossPVPC,[],1))]);
% axes('Fontsize',12);
xlabel('AC Current [A]rms','Fontsize',14);ylabel ('frequency [Hz]','Fontsize',14);
zlabel('Q [J/cycle/m^3]','Fontsize',14);
set(gca,'FontSize',13)
end