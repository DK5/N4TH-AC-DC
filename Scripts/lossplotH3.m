...
figure ();h=surf(X,Y,1000.*data.lossH3','FaceColor','interp','FaceLighting','gouraud');
title (data.runtitle, 'Interpreter', 'none','Fontsize',14); 
axis ([min(data.current) max(data.current) 200 max(data.frequency) 1100*min(min(data.lossH3,[],1)) 1100*max(max(data.lossH3,[],1))]);
% axes('Fontsize',12);
